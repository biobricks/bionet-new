...
module.exports = function (Component) {
  ...
  return class ActionNavBar extends Component {
    ...
    editItemListener(item) {
      ...
      const promptComponent = (
        <EditPhysical 
          state="EditPhysical" 
          active="true" 
          item={app.state.inventory.physicalItem} 
        />
      );
    ...
    addItemClick(e) {
      ...
      let x = parent.parent_x;
      let y = parent.parent_y;
    ...
    openAddItemMenu() {
      this.setState({
        enableAddItemMenu: (!this.state.enableAddItemMenu) ? 'is-active' : '' 
      });
    }
    ...
    openFavoritesMenu() {
      this.setState({
        enableFavoritesMenu: (!this.state.enableFavoritesMenu) ? 'is-active' : '' 
      });
    }
    ...
    editItem() {
      ...
      if (!app.state.inventory.selection || !app.state.inventory.selection.id) {
        item = this.generateNewItem(
          app.state.inventory.selection.parentId,
          app.state.inventory.selection.x,
          app.state.inventory.selection.y,
          'container'
        );
      } else {
      ...
      app.actions.prompt.display('Do you wish to delete ' + name + '?', null, function(accept) {
        if (accept) {
          app.actions.inventory.delPhysical(id, function(err,id2) {
            ...
            app.actions.inventory.selectCell(
              parentItem.id, 
    ...
    render() {
      const initMenu = function() {
        ...
        const DropdownMenuItem = function(props) {
          return ( 
            <a 
              id={props.id} 
              class={'dropdown-item ' + ((props.emphasis) ? 'bold' : '')} 
        ...
        menu.push(
          <DropdownMenuItem 
            label="Existing Biomaterial" 
            emphasis 
            onClick={this.addExisting.bind(this)} 
          />
        );
        for (var i=0; i<menuDef.length; i++) {
          ...
          menu.push(
            <DropdownMenuItem 
              id={item.name} 
              label={item.title} 
              onClick={this.addItemClick.bind(this)} 
            />
          );
        }
        return menu;
      ...
      return (
        <div 
          ...
          class="tile is-1 is-vertical" 
          style={actionsContainerStyle}
        >
          <ActionMenuButton 
            icon="home" 
            onClick={this.homeItem.bind(this)} 
          />
          ...
          <div class={"dropdown tile "+this.state.enableAddItemMenu}>
            <div class="dropdown-trigger">
              <ActionMenuButton 
                icon="plus" 
                onClick={this.openAddItemMenu.bind(this)} 
              />
            </div>
          ...
          <div class={"dropdown " + this.state.enableFavoritesMenu}>
            <div class="dropdown-trigger">
              <ActionMenuButton 
                icon="star" 
                onClick={this.openFavoritesMenu.bind(this)} 
              />
            </div>
                ...
                <div class="dropdown-content"  style={"position:fixed;top:265px;left:7px;"}>
                  <div class="dropdown-item"  style="margin:0;padding:0;">
                    <Favorites 
                      ...
                      selectFunction={this.selectFavorite.bind(this)} 
                      addFunction={this.addFavorite.bind(this)} 
                      selectedItem={app.state.inventory.selection}
                    />
                  </div>
                </div>
              </div>
