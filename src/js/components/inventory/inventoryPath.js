...
module.exports = function (Component) {
  ...
  return class InventoryPath extends Component {
    ...
    print() {
      ...
      const callback = function(err, labelImageData, labelData) {
        ...
        app.actions.inventory.saveToInventory(item, labelImageData, true, function(err) {
          if(err) {
            return app.actions.notify(err, "error");
          }  
      ...
      const promptComponent = (
        <PrintLabel callback={callback} item={item}/>
      );
    ...
    onSaveNew(dbData, type) {
      if (type === 'physical') {
        this.onSaveNewPhysical(dbData);
      } else {
        this.onSaveNewContainer(dbData);
      }  
    }
    ...
    onEditPanelMount(width,height) {
      ...
      this.setState({
        editPanelWidth: width,
        editPanelHeight: height
      });
    }
    ...
    render() {
      ...
      if (!this.props.inventoryPath || !this.props.id) {
        return;
      }  
      ...
      if (this.state.editMode) {
        ...
        let editPanelClass = 'is-12-desktop';
        if (!fullWidth) {
        ...
        if (this.state.layoutWidthUnits) {
          location.layoutWidthUnits = this.state.layoutWidthUnits;
        }  
        if (this.state.layoutHeightUnits) {
          location.layoutHeightUnits = this.state.layoutHeightUnits;
        }  
        if (this.state.majorGridLine) {
          location.majorGridLine = this.state.majorGridLine;
        }  
        ...
        let panelWidth = this.state.mapPanelWidth;
        let panelHeight = this.state.mapPanelHeight;
        let zoomWidth = Math.min(200,this.state.mapPanelWidth);
        let zoomHeight = this.state.mapPanelHeight;
        ...
        if (currentItem.type !== 'lab') {
          ...
          const rootPath = rootPath2.filter(container => { 
            return container; 
          });
          if (this.state.highlightedRecord) {
            console.log('baseDisplayObject, highlighted', this.state.highlightedRecord);
          }  
          rootLocation = (
            <div>
              <LocationPath
        ...
        const locationPath = locationPath2.filter(container => { 
          return container; 
        });
        ...
        let locationPathComponent = null;
        ...
        if(this.state.suggestMode) {
          ...
          if(this.state.suggestMode.loading) {
            ...
            for(i = 0; i < this.state.suggestMode.results.length; i++) {
              ...
              results.push((
                <li>
                  <Link to={'/inventory/new/' + res.value.id}>
                    {res.value.name}
                  </Link>
                </li>
              ));
